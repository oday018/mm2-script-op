...
-- تحقق من إمكانية استخدام الـ Hook (موجودة بالفعل في الكود الأصلي)
...
-- زر لفتح قائمة اختيار اللاعب
FlingTab:AddButton({
  Name = "اختر لاعب لقذفه",
  Callback = function()
    -- الحصول على أسماء جميع اللاعبين
    local playerNames = {}
    for _, player in ipairs(Players:GetPlayers()) do
      if player ~= LocalPlayer then
    ...
    -- إذا لم يكن هناك لاعبين آخرين، أظهر إشعاراً
    if #playerNames == 0 then
      Window:Notify({
        Title = "⚠️ لا يوجد لاعبين آخرين",
        Content = "لا يوجد لاعبين آخرين لقذفهم",
        Duration = 3
      })
      return
    end
    ...
    -- انتظر حتى يتم اختيار لاعب أو إغلاق القائمة
    while dropdown:IsVisible() do
      task.wait()
    end
    
    -- إذا تم اختيار لاعب، قم بقذفه
    if selectedPlayerName then
      ...
      if player and player ~= LocalPlayer then
        ...
        Window:Notify({
          Title = "❌ خطأ",
          Content = "لم يتم العثور على اللاعب: " .. (selectedPlayerName or ""),
          Duration = 3
        })
      end
    else
      Window:Notify({
        Title = "⚠️ تم الإلغاء",
        Content = "لم يتم اختيار لاعب لقذفه",
        Duration = 3
      })
    end
  end
...
FlingTab:AddToggle({
    ...
    Callback = function(value)
        ...
        -- إعادة التصادم عند إيقاف الحماية
        ...
        -- تعطيل التصادم عند تفعيل الحماية
        task.spawn(function()
