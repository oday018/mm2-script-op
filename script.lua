...
local Window = OrionLib:MakeWindow({Name = "Murder Mustery 2", HidePremium = false, SaveConfig = true, ConfigFolder = "OrionTest"})
	...
	Callback = function()
        ...
        if character then
            ...
            if humanoidRootPart then
                local gunDrop = workspace.Normal:FindFirstChild("GunDrop") -- Изменение пути к объекту GunDrop
                if gunDrop then
                    ...
                    print("Объект 'GunDrop' не найден")
                end
            else
                print("Персонаж игрока не имеет 'HumanoidRootPart'")
            end
        else
...
Tab:AddButton({
	...
	Callback = function()
        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-110.062035, 139.419052, 8.29038811, -0.625465393, 0.0148606021, 0.780110419, -1.1343509e-06, 0.999818563, -0.0190468077, -0.780251861, -0.0119140204, -0.625351846)
  	end    
})
...
local Section = Tab:AddSection({
	Name = "Developer: TheMeetly"
})

local Section = Tab:AddSection({
	Name = "Beta-tester: TheMeetly, smetan_4ik"
})
